...
このプロジェクトで番号管理しているもの（シナリオ連番、タスク番号 T001〜、機能要件番号 FR-001〜、成功基準番号 SC-001〜 等）すべてに適用するルール:

**必須事項**:
- 番号付きの項目を削除した場合、欠番となった番号は再利用しない
- 欠番以降の番号をすべて繰り上げ（詰め）ることはしない
- 一度付与した番号は識別子として固定し、変更しない

**根拠**: 番号はコミットログ、レビューコメント、仕様書、チームのコミュニケーションにおける参照 ID として機能する。
番号を詰めると過去の参照が意味をなさなくなり、作業・要件の追跡可能性が損なわれる。

### spec フォルダ命名規則

新しい spec フォルダを作成する際の番号付けルール:

- `specs/` ディレクトリ内に既存するすべてのフォルダの番号を確認し、**グローバル最大番号 + 1** を使用する
- short-name の一致有無やリモートブランチの存在は考慮しない
- 番号は3桁のゼロパディングとする（例: `006-feature-name`）
- `specs/` ディレクトリが空の場合は `001` から開始する

**speckit コマンドとの優先関係**: `/speckit.specify` 等の speckit コマンドの内部指示がこのルールと矛盾する場合、**本憲章の規則を優先する**。speckit コマンドの指示は参考とし、番号付けは常にこの節の規則に従うこと。

**根拠**: short-name ごとに番号をリセットする方式はリモートブランチの状態に依存するため非決定的であり、番号の重複が発生しやすい。グローバル通し番号方式はシンプルで一貫性があり、フォルダ一覧から即座に次の番号が判断できる。
...
**Version**: 1.5.0 | **Ratified**: 2026-01-18 | **Last Amended**: 2026-02-24
