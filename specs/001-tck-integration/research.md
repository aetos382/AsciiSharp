...
**必要なステップ**:
1. asciidoc-tck サブモジュールを**最新版に更新**（`git submodule update --remote`）
   - **重要**: 更新に失敗した場合はワークフローを即座に失敗させる
...
**サブモジュール更新の要件**:
- 毎回の CI 実行時に asciidoc-tck サブモジュールを最新版に更新する
- サブモジュールの更新に失敗した場合、ワークフロー全体を失敗させる（後続ステップを実行しない）
- これにより、常に最新の TCK テストスイートに対してテストが実行される
...
- **サブモジュールの固定バージョン**: 常に最新版を使用する方針を採用。これにより TCK 仕様の変更に追従できる
...
| サブモジュール更新 | 毎回最新版に更新、失敗時はワークフロー失敗 | 常に最新 TCK テストに追従 |
