...
- **TCK サブモジュールの不在**: asciidoc-tck サブモジュールが初期化されていない場合、CI ワークフローでサブモジュールを初期化する
...
- **FR-008**: CI ワークフローは asciidoc-tck サブモジュールを初期化し、Node.js 20 以上をセットアップしなければならない
- **FR-009**: TCK テストの実行には `ASCIIDOC_TCK_ADAPTER` 環境変数でアダプターのパスを指定しなければならない
