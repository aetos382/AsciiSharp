...
- **CI 環境の問題**: GitHub Actions ランナーに必要なツールが存在しない場合、適切なエラー メッセージを出力して失敗する
...
- **FR-008**: TCK テストの実行には `--adapter-command` オプションでアダプターのパスを指定しなければならない
- **FR-009**: TCK テストの結果（成功/失敗）を明確に出力しなければならない
- **FR-010**: 失敗した TCK テストについて、失敗した構文要素を特定できる情報を提供しなければならない
