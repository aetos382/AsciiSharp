...
- **FR-008**: 各構文ノードは、ソース内での位置情報（開始位置、終了位置）を保持しなければならない
...
- **Document（文書）**: AsciiDoc 文書全体を表すルートエンティティ。ヘッダーとボディで構成される
- **Block（ブロック）**: 垂直に積み重なる構造単位。段落、セクション、リスト、テーブル、区切りブロックなど
- **Inline（インライン要素）**: ブロック内に存在するテキストレベルの要素。書式マークアップ、マクロ、属性参照
- **Trivia（トリビア）**: 空白、改行、コメントなど、意味に影響しないがソース再構築に必要な要素
- **SyntaxToken（構文トークン）**: 構文木の末端要素。キーワード、演算子、リテラル文字列など
- **SyntaxNode（構文ノード）**: 構文トークンと他のノードの組み合わせで構成される中間ノード
