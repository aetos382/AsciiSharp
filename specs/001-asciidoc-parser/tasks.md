...
- [ ] T008 SyntaxKind 列挙型を Source/AsciiSharp/SyntaxKind.cs に実装（Tokens, Trivia, Nodes 定義）
- [ ] T009 [P] TextSpan 構造体を Source/AsciiSharp/Text/TextSpan.cs に実装
...
- [ ] T026 [P] [US1] BasicParsing.feature を Test/AsciiSharp.Specs/Features/BasicParsing.feature に作成（Acceptance Scenarios 1-3 を Given-When-Then 形式で記述）
- [ ] T027 [US1] BasicParsingSteps.cs を Test/AsciiSharp.Specs/StepDefinitions/BasicParsingSteps.cs に作成（ステップ定義の実装）
- [ ] T028 [US1] BDD テストを実行し、Red（失敗）を確認

### Implementation for User Story 1（Green フェーズ）

**ドメインノードの実装**:

- [ ] T029 [P] [US1] DocumentSyntax クラスを Source/AsciiSharp/Syntax/DocumentSyntax.cs に実装
- [ ] T030 [P] [US1] DocumentHeaderSyntax クラスを Source/AsciiSharp/Syntax/DocumentHeaderSyntax.cs に実装
- [ ] T031 [P] [US1] DocumentBodySyntax クラスを Source/AsciiSharp/Syntax/DocumentBodySyntax.cs に実装
- [ ] T032 [P] [US1] SectionSyntax クラスを Source/AsciiSharp/Syntax/SectionSyntax.cs に実装
- [ ] T033 [P] [US1] SectionTitleSyntax クラスを Source/AsciiSharp/Syntax/SectionTitleSyntax.cs に実装
- [ ] T034 [P] [US1] ParagraphSyntax クラスを Source/AsciiSharp/Syntax/ParagraphSyntax.cs に実装
- [ ] T035 [P] [US1] UnorderedListSyntax クラスを Source/AsciiSharp/Syntax/UnorderedListSyntax.cs に実装
- [ ] T036 [P] [US1] OrderedListSyntax クラスを Source/AsciiSharp/Syntax/OrderedListSyntax.cs に実装
- [ ] T037 [P] [US1] ListItemSyntax クラスを Source/AsciiSharp/Syntax/ListItemSyntax.cs に実装
- [ ] T038 [P] [US1] TextSyntax クラス（インライン要素）を Source/AsciiSharp/Syntax/TextSyntax.cs に実装（基本テキストのみ）

**Lexer 実装**:

- [ ] T039 [US1] Lexer クラスを Source/AsciiSharp/Parser/Lexer.cs に実装（基本トークン: Text, NewLine, Whitespace, Equals, Asterisk, Dash をサポート）
- [ ] T040 [US1] Lexer に Trivia サポート（先行/後続トリビアの割り当て）を実装

**Parser 実装**:

- [ ] T041 [US1] Parser クラスを Source/AsciiSharp/Parser/Parser.cs に実装（イベントベース再帰下降パーサー、Document/Section/Paragraph/List をサポート）
- [ ] T042 [US1] Parser に ロスレス解析（空白・改行の完全保持）を実装
- [ ] T043 [US1] SyntaxTree.ParseText() メソッドを実装（エントリーポイント）

**検証**:

- [ ] T044 [US1] BDD テストを再実行し、Green（成功）を確認
- [ ] T045 [US1] ラウンドトリップ検証（構文木 → テキスト再構築）をテスト
- [ ] T046 [US1] ビルドを実行し、警告ゼロを確認

### Refactor for User Story 1

- [ ] T047 [US1] コードレビューと警告解消
- [ ] T048 [US1] Lexer/Parser の共通パターンをリファクタリング
...
- [ ] T049 [P] [US2] ErrorRecovery.feature を Test/AsciiSharp.Specs/Features/ErrorRecovery.feature に作成（Acceptance Scenarios 1-3）
- [ ] T050 [US2] ErrorRecoverySteps.cs を Test/AsciiSharp.Specs/StepDefinitions/ErrorRecoverySteps.cs に作成
- [ ] T051 [US2] BDD テストを実行し、Red を確認

### Implementation for User Story 2（Green フェーズ）

**エラー回復機能の実装**:

- [ ] T052 [P] [US2] MissingTokenSyntax クラス（欠落トークンを表現）を Source/AsciiSharp/Syntax/MissingTokenSyntax.cs に実装
- [ ] T053 [P] [US2] ErrorNodeSyntax クラス（エラー部分をラップ）を Source/AsciiSharp/Syntax/ErrorNodeSyntax.cs に実装
- [ ] T054 [US2] ParserRecovery クラスを Source/AsciiSharp/Parser/ParserRecovery.cs に実装（同期ポイント戦略: 空行、セクションヘッダー）
- [ ] T055 [US2] Parser にエラー回復ロジックを統合（スキップ・挿入・エラーノード生成）
- [ ] T056 [US2] Diagnostic の生成と SyntaxTree への集約を実装
- [ ] T057 [US2] エラーノードの IsMissing プロパティと ContainsDiagnostics フラグを実装

**検証**:

- [ ] T058 [US2] BDD テストを再実行し、Green を確認
- [ ] T059 [US2] 複数エラーを含む文書で正常部分が解析されることをテスト
- [ ] T060 [US2] ビルドを実行し、警告ゼロを確認

### Refactor for User Story 2

- [ ] T061 [US2] エラーメッセージの多言語化対応検討（将来拡張）
- [ ] T062 [US2] エラー回復ロジックのリファクタリング
...
- [ ] T063 [P] [US3] InlineMarkup.feature を Test/AsciiSharp.Specs/Features/InlineMarkup.feature に作成（Acceptance Scenarios 1-3）
- [ ] T064 [US3] InlineMarkupSteps.cs を Test/AsciiSharp.Specs/StepDefinitions/InlineMarkupSteps.cs に作成
- [ ] T065 [US3] BDD テストを実行し、Red を確認

### Implementation for User Story 3（Green フェーズ）

**インライン要素ノードの実装**:

- [ ] T066 [P] [US3] InlineSyntax 抽象クラスを Source/AsciiSharp/Syntax/InlineSyntax.cs に実装
- [ ] T067 [P] [US3] FormattedTextSyntax クラス（太字・斜体・等幅）を Source/AsciiSharp/Syntax/FormattedTextSyntax.cs に実装
- [ ] T068 [P] [US3] LinkSyntax クラスを Source/AsciiSharp/Syntax/LinkSyntax.cs に実装
- [ ] T069 [P] [US3] MacroSyntax クラスを Source/AsciiSharp/Syntax/MacroSyntax.cs に実装
- [ ] T070 [P] [US3] AttributeReferenceSyntax クラスを Source/AsciiSharp/Syntax/AttributeReferenceSyntax.cs に実装

**Lexer 拡張**:

- [ ] T071 [US3] Lexer にインラインマークアップトークンを追加（*, _, `, [, ], {, } など）

**Parser 拡張**:

- [ ] T072 [US3] Parser にインライン解析ロジックを追加（ParseInlines メソッド）
- [ ] T073 [US3] ネストしたインライン書式の解析を実装
- [ ] T074 [US3] URL マクロ、相互参照、脚注マクロの解析を実装

**検証**:

- [ ] T075 [US3] BDD テストを再実行し、Green を確認
- [ ] T076 [US3] ネストした書式のエッジケースをテスト
- [ ] T077 [US3] ビルドを実行し、警告ゼロを確認

### Refactor for User Story 3

- [ ] T078 [US3] インライン解析ロジックの最適化
- [ ] T079 [US3] 共通インラインパターンのリファクタリング
...
- [ ] T080 [P] [US4] Immutability.feature を Test/AsciiSharp.Specs/Features/Immutability.feature に作成（Acceptance Scenarios 1-3）
- [ ] T081 [US4] ImmutabilitySteps.cs を Test/AsciiSharp.Specs/StepDefinitions/ImmutabilitySteps.cs に作成
- [ ] T082 [US4] BDD テストを実行し、Red を確認

### Implementation for User Story 4（Green フェーズ）

**クエリ API の実装**:

- [ ] T083 [P] [US4] SyntaxNode にクエリメソッドを追加（DescendantNodes, DescendantTokens, Ancestors など）
- [ ] T084 [P] [US4] LINQ 拡張メソッドを Source/AsciiSharp/Syntax/SyntaxExtensions.cs に実装（OfKind, FirstAncestor など）

**変更 API の実装**:

- [ ] T085 [US4] SyntaxNode.ReplaceNode メソッドを実装（新しい構文木を返す）
- [ ] T086 [US4] SyntaxNode.InsertNodeBefore/After メソッドを実装
- [ ] T087 [US4] SyntaxNode.RemoveNode メソッドを実装
- [ ] T088 [US4] SyntaxTree.WithRootAndOptions メソッドを実装（構文木全体の置き換え）

**不変性保証のテスト**:

- [ ] T089 [US4] 構文木変更後に元のインスタンスが変更されないことをユニットテストで検証（Test/AsciiSharp.Tests/Syntax/ImmutabilityTests.cs）
- [ ] T090 [US4] BDD テストを再実行し、Green を確認
- [ ] T091 [US4] ビルドを実行し、警告ゼロを確認

### Refactor for User Story 4

- [ ] T092 [US4] クエリ API のパフォーマンス最適化
- [ ] T093 [US4] 変更 API の共通パターンをリファクタリング
...
- [ ] T094 [P] [US5] IncrementalParsing.feature を Test/AsciiSharp.Specs/Features/IncrementalParsing.feature に作成（Acceptance Scenarios 1-3）
- [ ] T095 [US5] IncrementalParsingSteps.cs を Test/AsciiSharp.Specs/StepDefinitions/IncrementalParsingSteps.cs に作成
- [ ] T096 [US5] BDD テストを実行し、Red を確認

### Implementation for User Story 5（Green フェーズ）

**増分解析インフラの実装**:

- [ ] T097 [P] [US5] TextChange 構造体を Source/AsciiSharp/Text/TextChange.cs に実装（変更範囲と新しいテキストを表現）
...
- [ ] T107 [US5] 増分解析ロジックの最適化
- [ ] T108 [US5] 構造共有のメモリ効率改善
...
- [ ] T109 [P] [US6] TableParsing.feature を Test/AsciiSharp.Specs/Features/TableParsing.feature に作成（Acceptance Scenarios 1-3）
- [ ] T110 [US6] TableParsingSteps.cs を Test/AsciiSharp.Specs/StepDefinitions/TableParsingSteps.cs に作成
- [ ] T111 [US6] BDD テストを実行し、Red を確認

### Implementation for User Story 6（Green フェーズ）

**テーブルノードの実装**:

- [ ] T112 [P] [US6] TableSyntax クラスを Source/AsciiSharp/Syntax/TableSyntax.cs に実装
- [ ] T113 [P] [US6] TableRowSyntax クラスを Source/AsciiSharp/Syntax/TableRowSyntax.cs に実装
- [ ] T114 [P] [US6] TableCellSyntax クラスを Source/AsciiSharp/Syntax/TableCellSyntax.cs に実装

**Lexer 拡張**:

- [ ] T115 [US6] Lexer にテーブル区切りトークンを追加（|===, |, , など）

**Parser 拡張**:

- [ ] T116 [US6] Parser にテーブル解析ロジックを追加（ParseTable メソッド）
- [ ] T117 [US6] CSV 形式のデータ解析を実装
- [ ] T118 [US6] セルのスパン情報（ColSpan, RowSpan）解析を実装
- [ ] T119 [US6] テーブルヘッダーの識別を実装

**検証**:

- [ ] T120 [US6] BDD テストを再実行し、Green を確認
- [ ] T121 [US6] 複雑なテーブル（結合セル、CSV）のエッジケースをテスト
- [ ] T122 [US6] ビルドを実行し、警告ゼロを確認

### Refactor for User Story 6

- [ ] T123 [US6] テーブル解析ロジックの最適化
- [ ] T124 [US6] CSV パーサーのリファクタリング
...
- [ ] T125 [P] DocumentHeaderSyntax に AuthorLine, RevisionLine, AttributeEntry のサポートを追加（Source/AsciiSharp/Syntax/）
- [ ] T126 [P] コメント解析（単一行 //, ブロック ////）を Source/AsciiSharp/Parser/Lexer.cs に実装
- [ ] T127 [P] include ディレクティブ、条件付きディレクティブの構文認識を追加（実際のファイル読み込みは行わない）
...
- [ ] T135 全体のコードレビューとリファクタリング
- [ ] T136 CI パイプライン設定（.NET 10 / .NET Framework 4.8.1 テスト、AOT 互換性チェック）
- [ ] T137 quickstart.md の検証シナリオを実行
- [ ] T138 最終ビルドと警告ゼロの確認
