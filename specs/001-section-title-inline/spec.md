...
- Q: `=` が 7 個以上の扱い → A: セクション見出しとして扱わず、本文テキスト（段落）とする。VSCode の AsciiDoc Extension のプレビュー表示でも Level 7 は見出しとして扱われないため
- Q: `=` の後に空白がない場合の扱い → A: セクション見出しとして扱わず、本文テキスト（段落）とする
...
2. **Given** `==タイトル`（空白なし）という入力、**When** パースする、**Then** セクション見出しではなく段落（ParagraphSyntax）として扱われ、テキスト復元も一致する
...
- マーカーと本文の間に空白が複数ある場合、それらすべてがトリビアとして保持される
- `=` が 7 個以上の場合（例: `======= Title`）、セクション見出しとして扱わず段落（本文テキスト）として扱う
- `=` の後に空白がない場合（例: `==タイトル`）、セクション見出しとして扱わず段落として扱う
...
- **FR-011**: `=` が 7 個以上の行はセクション見出しとして認識せず、段落（本文テキスト）として扱う
- **FR-012**: `=` の後に空白がない行（例: `==タイトル`）はセクション見出しとして認識せず、段落として扱う
...
- `Level` は 1〜6 の範囲とする。`=` が 7 個以上の場合はセクション見出しではなく段落として扱う。これは VSCode AsciiDoc Extension のプレビュー表示の挙動に準拠する
