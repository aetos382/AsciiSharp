...
フィーチャ: 基本的な AsciiDoc 文書の解析
    ...
    シナリオ: BOM を含む文書の解析
        ...
        ならば 構文木のルートは Document ノードである
        かつ ソーステキストは BOM を含む
        かつ Header のタイトルは "タイトル" である
        かつ BOM を含む元のテキストを復元できる

    @LineEnding
    シナリオ: CRLF 改行コードを含む文書の解析
        前提 CRLF 改行コードの以下の AsciiDoc 文書がある:
            """
            = タイトル
    ...
    @LineEnding
    シナリオ: CR 改行コードを含む文書の解析
        前提 CR 改行コードの以下の AsciiDoc 文書がある:
            """
            = タイトル
    ...
    @LineEnding
    シナリオ: 混在する改行コードを含む文書の解析
        前提 混在する改行コードの以下の AsciiDoc 文書がある:
            """
            = タイトル
